<template>
  <div class="min-h-[100vh] pb-20 bg-none">
    ...
    <!-- 계좌번호 + 총 자산 & 충전 버튼 -->
    <section class="flex items-center justify-between gap-6 mt-2 px-8 py-4">
      <!-- 로딩 중일 때 스켈레톤 UI -->
            ...
            <div class="flex items-center mb-2 gap-2">
              ...
              <div class="flex justify-between items-center flex-1">
                ...
                <span class="text-xs font-bold text-gray-400"
                  >{{ portfolioPercentages.cash || 0 }}%</span
                >
              </div>
            </div>
          </div>
...
watch(
  ...
  ([loaded]) => {
    if (loaded) {
      nextTick(() => {
        updatePortfolioChart(portfolioPercentages.value, userAccount.value.currentBalance)
      })
    }
  },
...
const onChargeNext = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      const chargedPoint = chargeCreditInput.value
      await loadUserData()
