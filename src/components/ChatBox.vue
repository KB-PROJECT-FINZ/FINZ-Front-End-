...
async function fetchGPT(prompt, explicitIntent = null) {
  ...
  let intentType = null
  if (explicitIntent) intentType = explicitIntent
  else if (awaitingTermExplain.value) intentType = 'TERM_EXPLAIN'
  else if (awaitingKeyword.value) intentType = 'RECOMMEND_KEYWORD'
  else if (awaitingStockAnalyze.value) intentType = 'STOCK_ANALYZE'
  else if (props.fixedIntent) intentType = props.fixedIntent
  else if (typeof chatStore.intentType === 'string') intentType = chatStore.intentType
